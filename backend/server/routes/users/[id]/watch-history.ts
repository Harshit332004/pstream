...
export default defineEventHandler(async event => {
  ...
  if (method === 'GET') {
    try {
      ...
      console.warn('watch-history GET failed (likely no DB):', e?.message?.substring(0, 80));
      return [];
    }
  }
  ...
  if (method === 'DELETE') {
    try {
      ...
      console.warn('watch-history DELETE failed (likely no DB):', e?.message?.substring(0, 80));
